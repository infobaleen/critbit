...
// MapStringValueType implements an associative array of ValueType indexed by string.
type MapStringValueType struct {
	length int
	root   *nodeMapStringValueType
}

// NewMapStringValueType returns a new map with keys of type string and values of type ValueType
func NewMapStringValueType() *MapStringValueType {
	var r MapStringValueType
	return &r
}

func (MapStringValueType) allocateNode() {
	var baseSize = C.sizeof_nodeMapStringValueType{}
	C.malloc(baseSize)
}

type nodeMapStringValueType struct {
	crit  uint           // Position of critical bit  (LSB=0; ^uint(0) indicates leaf)
	child unsafe.Pointer // Pointer to children or value ([2]nodeMapStringValueType or ValueType)
}

type nodeMapStringValueTypeLeaf struct {
	crit  uint
	value ValueType
	key   [1]uint64
}

type nodeMapStringValueTypeInternal struct {
	crit  uint
...
func (c nodeMapStringValueType) dir(key string) int {
...
func (c nodeMapStringValueType) findCrit(key string) uint {
...
func (c *nodeMapStringValueType) children() *[2]nodeMapStringValueType {
	return (*[2]nodeMapStringValueType)(c.child)
}

func (c *nodeMapStringValueType) value() *ValueType {
...
// parent is nil.
func (c *nodeMapStringValueType) find(key string) (crit uint, child, parent *nodeMapStringValueType) {
	child = c
...
func (t *MapStringValueType) transformKey(key string) string {
	var mask string = 1
	if mask-2 < 0 {
		mask = mask << 7
		for mask > 0 {
...
func (t *MapStringValueType) Rem(key string) {
	if t.length == 0 {
		return
	}
...
func (t *MapStringValueType) SetP(key string, val *ValueType) {
	if val == nil {
		t.Rem(key)
		return
	}
	...
	var children = [2]nodeMapStringValueType{*n, *n}
...
func (t *MapStringValueType) Set(key string, val ValueType) {
	t.SetP(key, &val)
}
...
func (t *MapStringValueType) GetP(key string) *ValueType {
	key = t.transformKey(key)
	if t.length > 0 {
		// Find leaf node
...
func (t *MapStringValueType) Get(key string) (ValueType, bool) {
	var v = t.GetP(key)
	if v == nil {
		var zero ValueType
		return zero, false
...
func (t *MapStringValueType) Length() int {
	return t.length
}

// func (c *nodeMapStringValueType) dbg(p string) {
// 	if c.crit != ^uint(0) {
// 		fmt.Printf(p+"Node: %08b %d\n", ((c.key>>c.crit)|1)<<c.crit, c.crit)
// 		p += "  "
// 		var children = (*[2]nodeMapStringValueType)(c.child)
// 		children[0].dbg(p)
// 		children[1].dbg(p)
// 	} else {
...
// IterStringValueType The iterator becomes invalid
// if a new value is inserted in the underlying map, until the Reset or Jump method is called.
type IterStringValueType struct {
	t       *MapStringValueType
	nodes   []*nodeMapStringValueType
	lastDir int
	Key     string     // Key found by last call to Next, Prev.
...
// Iterator returns a new IterStringValueType.
func (t *MapStringValueType) Iterator() *IterStringValueType {
	var i IterStringValueType
	i.t = t
	i.Reset()
	return &i
}
...
func (i *IterStringValueType) Seek(key string) {
...
func (i *IterStringValueType) Reset() {
	i.Key = i.t.transformKey(0)
	i.Value = nil
	i.lastDir = 2
	if i.nodes == nil {
		i.nodes = make([]*nodeMapStringValueType, 0, 64)
	} else {
		i.nodes = i.nodes[0:0]
	}
}
...
func (i *IterStringValueType) Next() bool {
	i.step(1)
	return i.Value != nil
}
...
func (i *IterStringValueType) Prev() bool {
	i.step(0)
	return i.Value != nil
}

func (i *IterStringValueType) step(dir int) {
